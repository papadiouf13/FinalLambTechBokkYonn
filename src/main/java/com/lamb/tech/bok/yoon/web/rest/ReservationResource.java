...
import com.lamb.tech.bok.yoon.repository.ReservationRepository;
...
/**
 * REST controller for managing {@link com.lamb.tech.bok.yoon.domain.Reservation}.
 */
@RestController
...
public class ReservationResource {
    ...
    public ResponseEntity<Reservation> partialUpdateReservation(
        ...
        Optional<Reservation> result = reservationRepository
            ...
            .map(existingReservation -> {
                if (reservation.getDepartureTimme() != null) {
                    existingReservation.setDepartureTimme(reservation.getDepartureTimme());
                }
     ...
     */
    @GetMapping("")
    public List<Reservation> getAllReservations(@RequestParam(name = "filter", required = false) String filter) {
        if ("payment-is-null".equals(filter)) {
            log.debug("REST request to get all Reservations where payment is null");
